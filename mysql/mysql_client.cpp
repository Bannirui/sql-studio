...
void MySQLClient::process_handshake_v10_packet(const std::vector<uint8_t>& packet)
{
	...
	if (this->server_capabilities & CLIENT_PROTOCOL_41)
	{
		...
		auth_plugin_data.insert(auth_plugin_data.end(),
			handshake_v10_payload->auth_plugin_data_part_2.begin(),
			handshake_v10_payload->auth_plugin_data_part_2.end());
		...
		// TODO: 2024/7/31 重新编码前4byte
		size_t i = 0;
		std::cout << "修改前 " << std::endl;
		for (auto x : packet_bytes)
			std::cout << std::hex << std::setw(2) << std::setfill('0') << static_cast<uint32_t>(x) << " ";
		std::cout << std::dec << std::endl;
		ParserUtil::write_u24_to_byte_arr(packet_bytes, i, packet_bytes.size());
		packet_bytes[3] = m_packet.sequence_id + 1;
