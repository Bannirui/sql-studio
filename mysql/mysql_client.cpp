...
void MySQLClient::process_error_packet(const std::vector<uint8_t>& packet)
{
	...
	std::cout << "seq_id=" << static_cast<uint32_t>(seq_id) << std::endl;
	...
	std::string err_msg = ParserUtil::read_string_till_terminate(packet, i);
...
void MySQLClient::process_handshake_v10_packet(const std::vector<uint8_t>& packet)
{
	...
	MySQLPacket r_packet(&payload);
	r_packet.read(packet);
	r_packet.print();
	auto* handshake_v10_payload = (HandshakeV10Payload*)(r_packet.payload);
	...
	if (this->server_capabilities & CLIENT_PROTOCOL_41)
	{
		...
		// 握手包返回过来了21个字节 sha算法需要20个字节
		std::vector<uint8_t> auth_plugin_data;
		// 前8字节
		auth_plugin_data.insert(auth_plugin_data.end(),
			handshake_v10_payload->auth_plugin_data_part_1.begin(),
			handshake_v10_payload->auth_plugin_data_part_1.end());
		// 后12字节
		for (int i = 0; i < 12; i++)
		{
			auth_plugin_data.push_back(handshake_v10_payload->auth_plugin_data_part_2.at(i));
		}
		std::vector<uint8_t> auth_response = SHAUtil::scramble(auth_plugin_data, this->pwd);
		std::string auth;
		auth.insert(auth.end(), auth_response.begin(), auth_response.end());
		if (!(this->server_capabilities & CLIENT_PLUGIN_AUTH_LENENC_CLIENT_DATA))
		{
			// int<1>	auth_response_length	length of auth_response
			handshake_response_payload.auth_response_length = auth.length();
		}
		// string<length>	auth_response	opaque authentication response data generated by Authentication Method indicated by the plugin name field.
		handshake_response_payload.auth_response = auth;
		...
		handshake_response_payload.client_plugin_name = handshake_v10_payload->auth_plugin_name;
		// int<1>	zstd_compression_level	compression level for zstd compression algorithm
		handshake_response_payload.zstd_compression_level = 0x01;
		MySQLPacket handshake_resp_packet(&handshake_response_payload);
		handshake_resp_packet.sequence_id = r_packet.sequence_id + 1;
		std::vector<uint8_t> w_packet = handshake_resp_packet.write();
		this->print_packet(w_packet);
		this->send_packet(w_packet);
	}
	else
	{
		// TODO: 2024/7/21
